...
/// Process-local counter used to mint unique temp filenames for
/// concurrent synthesis calls. Combined with `process::id()` so two
/// running app instances can't collide either.
...
/// Mint a unique temp file path for one synthesis call. Pid + an
/// atomic counter so concurrent synth requests (rapid clicks across
/// multiple verses) don't collide on the same temp file. The file is
/// best-effort cleaned up at the end of synthesize(); a stray file
/// left after a crash is harmless (the OS cleans the temp dir).
...
/// Run Piper on `text` and return the bytes of the WAV file it wrote.
///
/// We pass Piper a real file path rather than `--output_file -` (which
/// it interprets as stdout). Stdout output is unreliable: depending on
/// build, it's either raw PCM samples with no header or a WAV header
/// with bogus length fields (Piper can't seek back to fix the size on
/// a stream). Either way the browser ends up guessing at format and
/// produces clipped/distorted/maxed-volume playback. A real file lets
/// Piper write a properly framed WAV that any decoder reads correctly.
///
...
pub fn synthesize(data_dir: &Path, voice_id: &str, text: &str) -> Result<Vec<u8>, String> {
    ...
    let temp_path = next_temp_wav_path();
    ...
    // Piper still emits progress messages on stdout/stderr even when
    // writing audio to a file. Drain both so it doesn't block on a
    // full pipe buffer.
    ...
    let stderr_buf = stderr_thread.join().ok().and_then(|r| r.ok()).unwrap_or_default();
    ...
    let bytes = match fs::read(&temp_path) {
        Ok(b) => b,
        Err(e) => {
            warn!("Piper stderr was: {}", stderr_buf);
            return Err(format!("read piper output {temp_path:?}: {e}"));
        }
    };
    let _ = fs::remove_file(&temp_path);
